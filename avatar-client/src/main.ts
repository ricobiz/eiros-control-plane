...
const renderer=new AvatarRenderer(stage); const behavior=new BehaviorEngine(42); const simulator=new LocalAvatarSimulator(); let adapter:VrmAdapter|null=null; let last=performance.now();
...
function loop(now:number){const dt=Math.min(.05,(now-last)/1000);last=now;const b=behavior.tick(now,dt*1000);const frame=simulator.next(now,controls,b);adapter?.apply(frame);renderer.render(dt);requestAnimationFrame(loop);}requestAnimationFrame(loop);
