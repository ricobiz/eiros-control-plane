...
const renderer=new AvatarRenderer(stage); const behavior=new BehaviorEngine(42); const simulator=new LocalAvatarSimulator(); const transport=new LocalTransport(); const stream=new AvatarStreamController(80); stream.connect(transport); let adapter:VrmAdapter|null=null; let last=performance.now();
...
function loop(now:number){const dt=Math.min(.05,(now-last)/1000);last=now;const b=behavior.tick(now,dt*1000);const frame=simulator.next(now,controls,b);transport.emitFrame(frame);const presented=stream.sample(frame.audioPtsMs+80);if(presented)adapter?.apply(presented);renderer.render(dt);requestAnimationFrame(loop);}requestAnimationFrame(loop);
